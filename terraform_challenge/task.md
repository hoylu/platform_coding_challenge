...
You are tasked to write terraform code to start multiple docker containers. Two docker containers running a nginx image and three running a plain ubuntu image. The nginx containers should be configured with each a different DNS configuration, a persistent docker volume and access to the internet as well as a private docker network. The ubuntu containers should be configured with a persistent volume and access to the same private docker network. The ubuntu containers should be kept running. The provided `.tfvars` file can be used as a starting point for this task.
